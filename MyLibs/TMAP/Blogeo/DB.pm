...

{
	...
	sub DBConnect {
		my $self = shift;
		#my $dsn = "DBI:Pg:dbname=blogeo;host=localhost;port=5432";
		my $dsn = "DBI:Pg:dbname=blogeo;host=192.168.0.103;port=5432";
		my $user = "postgres";
		my $pass = "psql";
	...
	sub register {
		...
		eval{
			$sth = $db{ident $self}->prepare($sql);
			$rv = $sth->execute(@bind) or return 0;
			$sth->finish();
		};
		if($@){
			if($DBI::errstr =~ m/duplicate.*/){
				$rv = 0;
			}
			else{
				die $@;
	...
	sub fetch {
		...
		my $rv = $sth->execute(@bind) or die $DBI::errstr;
